...
import com.example.planetapp.presentation.adapter.CrewAdapter
...
class CrewListActivity : AppCompatActivity() {
    ...
    private fun setupUI() {
        ...
        binding.titleText.text = "Crew Members"
    }
    
    private fun setupRecyclerView() {
        adapter = CrewAdapter()
        binding.recyclerView.apply {
            this.adapter = this@CrewListActivity.adapter
            layoutManager = LinearLayoutManager(this@CrewListActivity)
    ...
    private fun loadData() {
        ...
        lifecycleScope.launch {
            try {
                val response = SpaceXRetrofitClient.spaceXApi.getCrew()
                
                if (response.isSuccessful && response.body() != null) {
                    val crew = response.body()!!
                    if (crew.isEmpty()) {
                        showEmpty()
                    } else {
                        showContent(crew)
    ...
    private fun showContent(crew: List<CrewMember>) {
        ...
        binding.subtitleText.text = "${crew.size} astronaut${if (crew.size != 1) "s" else ""}"
    ...
    private fun showEmpty() {
        ...
        binding.errorLayout.visibility = View.GONE
        binding.subtitleText.text = "No crew found"
    }
